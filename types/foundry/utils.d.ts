...
type ConfiguredModuleData<Name extends string> = Name extends keyof ModuleConfig
    ? ModuleConfig[Name]
    : EmptyObject;
 ...
 */
export type FixedInstanceType<T extends AnyConstructor> =
    T extends abstract new (...args: infer _) => infer R ? R : never;
...
export type ConfiguredModule<Name extends string> =
    Name extends keyof RequiredModules
        ...
        :
              ...
              | ({ active: false } & Record<
                    keyof ConfiguredModuleData<Name>,
                    undefined
                >);
...
export type GetKey<T, K extends PropertyKey, D = never> = _GetKey<
    { _?: unknown } & T,
    K,
    D
>;

type _GetKey<T, K extends PropertyKey, D> = T extends {
    readonly [_ in K]?: infer V;
}
    ? V
    : D;
 ...
 * ```
 */
export type OverlapsWith<T, U> = [Extract<T, U>, any] extends [U, Extract<T, U>]
    ? T
    : U extends T
    ? T
    : U;
 ...
 * ```
 */
export type ArrayOverlaps<T, Item> = Extract<
    T,
    readonly unknown[]
> extends readonly Item[]
    ? OverlapsWith<T, readonly Item[]>
    : readonly Item[];
 ...
 */
export type MakeConform<T, ConformTo, D extends ConformTo = ConformTo> = [
    T
] extends [ConformTo]
    ? T
    : D;
 ...
 */
export type FolderDocumentTypes = Exclude<
    foundry.CONST.FOLDER_DOCUMENT_TYPES,
    'Compendium'
>;
 ...
 * @internal
 */
export type InexactPartial<
    T extends object,
    K extends AllKeysOf<T> = AllKeysOf<T>
> = {
...
export type Expanded<O> = O extends AnyObject
    ? {
          [KO in keyof O as KO extends `${infer A}.${string}`
              ? A
              : KO]: KO extends `${string}.${infer B}`
 ...
 */
export type Merge<T, U> = IsObject<U> extends true
    ? IsObject<T> extends true
        ? SimpleMerge<
              T,
              {
                  [K in keyof U]: T extends { readonly [_ in K]?: infer V }
                      ? Merge<V, U[K]>
                      : U[K];
              }
          >
...
export type Mixin<
    MixinClass extends AnyConcreteConstructor,
...
export type GetDataReturnType<T extends object> =
    GetDataConfigOptions<T>[GetDataConfigOption];
...
export type ShapeWithIndexSignature<
    ...
    IndexType
> = PrimaryShape & {
    readonly [K in keyof T & IndexSignature]: K extends keyof PrimaryShape
        ? PrimaryShape[K]
        : IndexType;
};
...
export type MustBeValidUuid<
    Uuid extends string,
    Type extends Document.Type = Document.Type
> = _MustBeValidUuid<Uuid, Uuid, Type>;
 ...
 * ```
 */
export type Quote<T extends string> = T extends `${string}'${string}`
    ? `'${Escape<T>}'`
    : `'${T}'`;

type Escape<T extends string> = T extends `${infer Prefix}'${infer Suffix}`
 ...
 * ```
 */
export type Coalesce<T, D, CoalesceType = undefined> = T extends CoalesceType
    ? D
    : T;
...
> = Extract<HooksRan<ValidHooksRan>, InitializedOnHook> extends never
    ? InitializedTo
